...
int get_ethernet_interface_name()
{
    ...
    { 
        ...
        //// No "Ethernet". Error !!!
        if(strstr(eth, "Ethernet") == NULL)
        { 
            bzero(eth_name, 20); 
            printf("[Main] Get Ethernet interface name error !!!\n"); 
            return 1;
        }
        else
        {
            printf("[Main] Ethernet:%s\n", eth_name); 
        }
        // close file
        fclose(fp); 
        return 0;
